...
from distiller_zoo import PKT, ABLoss, FactorTransfer, KDSVD, FSP, NSTLoss, KDN
from crd.criterion import CRDLoss

from helper.meta_loops import train_meta_distill, validate
...
def parse_option():
    ...
    parser.add_argument('--loss_lr', type=float, default=0.001, help='teacher learning rate')
    ...
    parser.add_argument('--path_s', type=str, default="", help='student model snapshot')
...
def main():
    ...
    criterion_ce_mse = KDN.CE_MSE(opt.alpha, opt.kd_T)
    ...
    criterion_list.append(criterion_ce_mse)
    ...
    loss_optimizer = optim.SGD(criterion_ce_mse.parameters(),
                            lr=opt.loss_lr,
                            momentum=opt.momentum,
                            weight_decay=opt.weight_decay)
    ...
    # for param_group in loss_optimizer.param_groups:
    #     param_group['lr'] = 0
    # routine
    for epoch in range(1, opt.epochs + 1):
        ...
        time1 = time.time()
        train_acc, train_loss = train_meta_distill(epoch, train_loader, held_loader, module_list, criterion_list, s_optimizer, loss_optimizer, opt)
        time2 = time.time()
