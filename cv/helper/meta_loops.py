...
def train_meta_distill(epoch, train_loader, held_loader, module_list, criterion_list, s_optimizer, loss_optimizer, opt):
    """One epoch distillation"""

    criterion_cls = criterion_list[0]
    criterion_kd = criterion_list[1]
    criterion_ce_mse = criterion_list[2]
    ...
    for d_idx, d_data in enumerate(train_loader):
        ...
        #########################################
        #  Step 2: Train Loss Network with S' on HELD set  #
        #########################################
        ...
        loss_grads = torch.autograd.grad(s_prime_loss, criterion_ce_mse.parameters())

        for p, gr in zip(criterion_ce_mse.parameters(), loss_grads):
            p.grad = gr
        ...
        temp_clamped = torch.clamp(criterion_ce_mse.temp.data, 1,6)
